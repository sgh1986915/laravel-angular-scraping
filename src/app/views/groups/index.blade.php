...
<div class="row">
  <div class="col-md-12">
	<div class="table-responsive">
		<table class="table table-striped table-hover">
			...
			@foreach ($groups as $group)
				<tr>
					...
					<td>
						<button class="btn btn-xs btn-default" onClick="location.href='{{ action('GroupController@edit', array($group->id)) }}'">{{trans('pages.actionedit')}}</button>
					 	<button class="btn btn-xs btn-default action_confirm {{ ($group->id == 2) ? 'disabled' : '' }}" type="button" data-method="delete" href="{{ URL::to('groups') }}/{{ $group->id }}">{{trans('pages.actiondelete')}}</button>
					 </td>
				</tr>	
			@endforeach
			</tbody>
	 ...
	 <button class="btn btn-xs btn-primary" onClick="location.href='{{ URL::to('groups/create') }}'">{{trans('groups.create')}}</button>
   </div>
</div>
<!--  
